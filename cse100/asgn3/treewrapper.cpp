...
int main(int argc, char **argv)
{
    ...
    char *com, *valstr, *op, *val; // for using with strtok, strtol
    //string val = 0; // the value from the command
    ...
    while (getline(input, command)) // get next line of input, store as repeat
    {
        ...
        //cout<<(string)valstr<<endl;
        val = valstr;

        if (strcmp(op, "i") == 0) // insert into list
        {
            //cout << "Insert " + (string)val << endl;
            myBST.insert(val);
        }
        if (strcmp(op, "d") == 0) // delete from list
        {
            //cout << "Delete " + (string)val << endl;
        ...
        if (strcmp(op, "f") == 0) // delete from list
        {
            ...
            if (myBST.find(val)) // yes, val is found in BST
                ...
                message = " not found";
            //cout << (string)val + message << endl;
            output << (string)val + message << endl;
        }
        ...
        if (strcmp(op, "r") == 0) // delete from list
        {
            string start = val;
            string end = strtok(NULL, " ");
            //cout << start << " " << end << endl;
            output << myBST.range(start,end)<<endl;
            //cout << myBST.range(start,end)<<endl;
            //cout << myBST.printInOrder(start,end)<<endl<<endl;
            continue;
