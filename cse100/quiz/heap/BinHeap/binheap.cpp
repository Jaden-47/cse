...
void BinHeap ::sink(int index)
{
    ...
    if (index >= size || (A[index] >= A[l] && A[index] >= A[r])) // base case
    {
        return;
    }
    ...
    if (A[l] > A[r])
    {
        if (A[index] < A[l]) // if value smaller than left leaf
        {
            int temp = A[index];
            A[index] = A[l];
            A[l] = temp;
    ...
    {
        if (A[index] < A[r]) // if value smaller than right leaf
        {
            int temp = A[index];
            A[index] = A[r];
            A[r] = temp;
